...
class DetailOverviewTab(nodes_tabs.DetailOverviewTab):
    ...
    def get_context_data(self, request):
        ...
        for param in [SAT_HOST_PARAM, SAT_AUTH_PARAM, SAT_ORG_PARAM]:
            if param not in stack.parameters:
                horizon.messages.error(request,
                                       _("No Satellite configuration found."))
                return result

        host = stack.parameters[SAT_HOST_PARAM]
        # Use for debugging:
        # host = 'http://sat-perf-04.idm.lab.bos.redhat.com'

        auth = stack.parameters[SAT_AUTH_PARAM].split(':')
        if auth[0] == 'oauth':
        ...
        # auth = ('admin', 'changeme')  # Use for debugging

        auth = stack.parameters[SAT_ORG_PARAM]
        # organization = '1-default_organization'  # Use for debugging
