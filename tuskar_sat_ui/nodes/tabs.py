...
class DetailOverviewTab(nodes_tabs.DetailOverviewTab):
    ...
    def get_context_data(self, request):
        ...
        # If the Satellite UUID isn't the same as this one, and it probably isn't
        # we need to store a mapping somewhere.
        search_url = '{host}/katello/api/v2/systems?search=' \
            'facts.net.interface.{iface}.mac_address:{mac}'
        for interface in ['eth0', 'eth1', 'en0', 'en1']:
            url = search_url.format(host=host, iface=interface, mac=mac)
