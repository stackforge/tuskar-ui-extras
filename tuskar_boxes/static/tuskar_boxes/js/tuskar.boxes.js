  ...
  module.init = function () {
    ...
    function updateBoxes() {
      $("div.boxes-flavor").each(function () {
          var $flavor = $(this);
          var roles = getRoleCounts($flavor);
          var roleNames = Object.getOwnPropertyNames(roles);
          var count = 0;
          var role = 0;
          $flavor.find("div.boxes-nodes div.boxes-node").each(function () {
            ...
            while (count >= roles[roleNames[role]]) {
              role += 1;
              count = 0;
            }
            if (!roleNames[role]) {
              ...
              $this.addClass("boxes-role-" + roleNames[role]).html("&nbsp;");
            }
            count += 1;
          });
      });
    ...
    $("div.boxes-drop").droppable({
        ...
        drop: function (ev, ui) {
          ...
          window.setTimeout(updateBoxes, 0);
        }
    });
    ...
    updateBoxes();
    $("input.number-picker").change(updateBoxes);
